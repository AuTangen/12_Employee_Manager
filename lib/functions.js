...
function updateEmployee() {
    inquirer.prompt(updateInq).then((data) => {
        ...
        connection.query('UPDATE employees SET role_id = ? WHERE id = ?', [updateRole, employeeid], function (err, newrole) {
            if(err) throw err;
...
function displayTable(data) {
    ...
    if (data.portal === 'View All Employees') {
        connection.query('SELECT employees.id, employees.first_name, employees.last_name, roles.title AS role, roles.salary, employees.manager_id AS manager FROM employees INNER JOIN roles ON employees.role_id=roles.id', function (err, employees) {
